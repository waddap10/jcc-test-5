...
export default function KanitOrderShow() {
    const { order, flash } = usePage<PageProps>().props;
    const { processing, patch } = useForm();

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Kanit Dashboard', href: route('kanit.dashboard') },
    ...
    const handleAccKanit = () => {
        if (confirm(`Are you sure you want to approve order ${order.custom_code}?`)) {
            patch(route('kanit.acc-kanit', order.id), {
                preserveScroll: true,
                onSuccess: (page) => {
                    const message = page.props.flash?.message;
                    console.log('Success:', message); // Shows file code
                }
            });
        }
    };
